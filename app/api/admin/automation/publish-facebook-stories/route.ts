...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await runWithAutomationLog('publish-facebook-stories', () =>
      publishPendingStoriesBatch('facebook')
    );
    return NextResponse.json(result);
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Falha na publicação automatizada de Stories no Facebook.' },
      { status: 500 }
    );
  }
}
