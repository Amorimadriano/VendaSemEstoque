...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${process.env.SUPABASE_URL}/rest/v1/admin_users?select=id,email,password_hash,role,active&email=eq.${encodeURIComponent(email)}&limit=1`, {
    headers: { apikey: process.env.SUPABASE_SERVICE_ROLE_KEY || '', Authorization: `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY || ''}` },
  });
  const admins = await response.json() as Array<{ id: string; email: string; password_hash: string; role: string; active: boolean }>;
